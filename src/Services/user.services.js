...
const register = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      // const isValidFirstName = /^[a-zA-Z-]*$/.test(userName);
      // if (!isValidFirstName) {
...
const logInUser = async (req, res) => {
  try {
    ...
    if (compare) {
      ...
      const data = { message: "User Loggedin successfully", result: { ...userData.toJSON(), token } };
      return data;
    } else {
...
const deleteUser = async (req, res) => {
  try {
    ...
    const deletedUser = await User.findByIdAndUpdate(
      { _id: id },
      { email: null, phoneNumber: null, socialId: null, isDeleted: true, otp: null }
    );
    return;
  } catch (error) {
...
const deactivatedUserByAdmin = async (req, res) => {
  try {
    const { id } = req.params;
    const user = await User.findOne({ _id: id });
    if (!user) {
    ...
    const deletedUser = await User.findByIdAndUpdate(
      { _id: id },
      { deactivatedByAdmin: true, isActive: false }
    );
    return;
  } catch (error) {
    const response = error;
...
//       throw new Error("User Not Found");
//     }
//     const loggedOutUser = await User.findOneAndUpdate({ _id: id },{deviceToken:null,deviceType:null,isActive:false},{new:true});
//     return
//   } catch (error) {
//     const response = error;
//     throw response;
